...
CreateBinnedData <- function(us, binlength, max_n_bin = NULL){
  rep <- USDataReport(us)
  
  n_bin = ceiling(min(rep) / binlength)
  if(!is.null(max_n_bin)){
    n_bin = min(max_n_bin, n_bin)
  }
  
  if(n_bin == 1){
    warning("data and bin paramteres result in a single bin, returning original data")
    return(us)
  }
  
  us_out <- CutUSdata(us,start = 0, maxlength = binlength)
  us_out$meta$bin <- "bin_1"
  us_out$meta$bin_int <- 1
  us_out$meta$parent_file <- rownames(us$meta)
  us_out <- AddPostFix(us_out,  "bin_1")
  
  for(i in 2:n_bin){
    us_bin <- CutUSdata(us,start = (i - 1) * binlength, maxlength = binlength)
    us_bin$meta$bin <- paste("bin_",i, sep = "")
    us_bin$meta$bin_int <- i
    us_bin$meta$parent_file <- rownames(us$meta)
    us_bin <- AddPostFix(us_bin,  paste("bin_",i, sep = ""))
    us_out <- FuseUSData(us_out, us_bin)
  }
...
AddPostFix <- function(us, postfix){
  for(i in 1:length(us$file_names)){
    us$file_names[i] <- paste(us$file_names[i], postfix, sep = "_")
  }
  for(i in 1:length(us$files)){
    names(us$files)[i] <- paste(names(us$files)[i], postfix, sep = "_")
  }
  for(i in 1:nrow(us$meta)){
    rownames(us$meta)[i] <- paste(rownames(us$meta)[i], postfix, sep = "_")
  }
  if(!is.null(us$Report)){
    for(i in names(us$Report)){
      for(j in 1:nrow(us$Report[[i]]))
        rownames(us$Report[[i]])[j] <- paste(rownames(us$Report[[i]])[j], postfix, sep = "_")
    } 
  }
  return(us)  
}
...
#' US <- PlotBehaviorFlow(US, lab = "kmeans.25")
#' US <- PlotBehaviorFlow(US, file = c("File1.csv","File2.csv","File3.csv") ,lab = "kmeans.25")
PlotBehaviorFlow_Network <- function(us,file = NULL, lab, min_avg_trans = 5, scale_edge = 0.5, min_node_size = 10, scale_node_size = 20, font_size = 40, cluster = TRUE){
  if(!us$has.updated.analysis$Transmat){
    print("no updated Tranisition data present. Generating Transition matrix data first")
    us <- AddTransitionMatrixData(us)
  }
  ...
  mat = mat / length(file)
  mat <- ifelse(mat < min_avg_trans, 0, mat)
  
  #filter out eliminated nodes
  filter <- (apply(mat, 2, sum) + apply(mat,1,sum)) > 0
  mat <- mat[filter,filter]
  
  
  nodes <- tibble(id = rownames(mat), label = rownames(mat))
  edges <- NULL
  for(i in rownames(mat)){
    for(j in rownames(mat)){
      if(mat[i,j] > 0){
        edges <- rbind(edges,data.frame(from = i, to = j, weight =mat[i,j]))
      }
    }
  }
  edges <- as_tibble(edges)
  edges <- mutate(edges, width = weight*scale_edge)
  
  graph <- graph_from_data_frame(edges, directed = FALSE)
  
  if(cluster){
    cluster <- cluster_louvain(graph)
    nodes$group <- membership(cluster)
  }
    
  nodes$size <- min_node_size + apply(mat, 2, sum) / max(apply(mat, 2, sum)) * scale_node_size
  nodes <- nodes %>% mutate(font.size = font_size)
...
TwoGroupAnalysis <- function(us,group, name = NULL, n_bootstraps = 1000, lab = NULL){
  ...
  for(r in names(us$Report)){
    ...
    for(i in names(Report)){
      ...
      nline <- data.frame(i, pval, mean(a), mean(b), mean(a) / mean(b), CD)
      names(nline) <- c("name","p",unique(group)[1], unique(group)[2], "FC","CohensD")
      res <- rbind(res,nline)
    }
  ...
  for(i in lev){
    for(j in lev){
      ...
      CohensD = cohensD(unlist(A),unlist(B))
      transitions <- rbind(transitions, data.frame(from = i, to = j, p.value = test$p.value, A = mean(unlist(A)), B = mean(unlist(B)) ,FC = mean(unlist(A)) / mean(unlist(B)), CohensD = CohensD))
      }, error = function(e){
      transitions <- rbind(transitions, data.frame(from = i, to = j, p.value = NA, A = mean(unlist(A)), B = mean(unlist(B)) ,FC = NA, CohensD = NA))
      })
      }
  }
  ...
  #Calculate effect size based on BFL ratio and cohens D
  print(group)
  us2 <- StratifyGroups(us,group, r)
  A <- us2$meta$ratio[group == unique(group)[1]]
  B <- us2$meta$ratio[group == unique(group)[2]]
  ...
  us$Results[[name]]$TransitionStats[[r]]$CohensD <- abs(cohensD(A,B))
...
TwoGroupAnalysis_Clusters <- function(us,group, name = NULL, n_bootstraps = 1000, lab = NULL){
  require(imputeTS)
  require(pracma)
  if(length(us$files) != length(group)){
    warning("grouping vectorn needs to have same length as number of files. can not perform analysis\n")
  ...
  us$Results[[name]]$ClusterStats <- list()
  for(r in lab){
    clustervec <- list()
    ind_counts <- grepl(".count", names(us$Report[[r]]))
    for(i in names(us$files)){
      ind_file <- grepl(i, rownames(us$Report[[r]]))
      clustervec[[i]] <- unlist(us$Report[[r]][ind_file, ind_counts])
      clustervec[[i]] <- na_replace(clustervec[[i]], fill = 0)
    }
    
    # bootstrapping based analysis of inter group distance of mean transition matrix values
    A <- clustervec[group == unique(group)[1]]
    B  <- clustervec[group == unique(group)[2]]
    
    # for (n in names(A)) {
    #   A[[n]] <- A[[n]][order(names(A[[n]]))]
    # }
    # for (n in names(B)) {
    #   B[[n]] <- B[[n]][order(names(B[[n]]))]
    # }
    
    a <- do.call(cbind, A)
    a <- array(a, dim=c(dim(a)[1], dim(a)[2]))
    ...
    for(i in 1:n_bootstraps){
      grp <- sample(group)
      A <- clustervec[grp == unique(group)[1]]
      B  <- clustervec[grp == unique(group)[2]]
    ...
    # Add Results to USData
    us$Results[[name]]$ClusterStats[[r]]$distance <- distance
    us$Results[[name]]$ClusterStats[[r]]$bootstraps <- bootstraps
    us$Results[[name]]$ClusterStats[[r]]$percentile <- sum(bootstraps < distance) / length(c(bootstraps,distance)) * 100
    us$Results[[name]]$ClusterStats[[r]]$sigma <- (distance - mean(bootstraps)) / sd(bootstraps)
    us$Results[[name]]$ClusterStats[[r]]$p.value <- (1 - pracma::erf(us$Results[[name]]$ClusterStats[[r]]$sigma / sqrt(2))) / 2
    # us$Results[[name]]$TransitionStats[[r]]$transitions <- transitions
  }
...
TwoGroupAnalysis_PCA_Report_LogReg <- function(us,group, cumulative_proportion = 0.9, normalize = TRUE, lab = "raw"){
  if(length(us$files) != length(group)){
    warning("grouping vectorn needs to have same length as number of files. can not perform analysis")
    return(us)
  }
  ...
  #build dataframe for linear model
  lmdat <- data.frame(group = grp, pcares$x[,1:n_comp])
  lmdat_shuffled <- lmdat
  lmdat_shuffled$group <- sample(lmdat$group)
  
  mod <- glm(data = lmdat,formula = group ~ ., family = binomial(link = "logit"))
  mod.null <- glm(data = lmdat, formula = group ~ 1, family = binomial(link = "logit"))
  mod.shuffled <- glm(data = lmdat_shuffled, formula = group ~ ., family = binomial(link = "logit"))
  mod.shuffled.null <- glm(data = lmdat_shuffled, formula = group ~ 1, family = binomial(link = "logit"))
  
  stats_modvsnull <- anova(mod, mod.null, test ="Chisq")
  stats_shuffledvsnull <- anova(mod.shuffled, mod.shuffled.null, test ="Chisq")
  stats_modvsshuffled <- anova(mod, mod.shuffled, test ="Chisq")
  
  out <- list(mod = mod,mod.null = mod.null,mod.shuffled = mod.shuffled, mod.shuffled.null = mod.shuffled.null,
              stats_modvsnull = stats_modvsnull,
              stats_shuffledvsnull = stats_shuffledvsnull,
              stats_modvsshuffled = stats_modvsshuffled)
  return(out)
}


#' Determines for each sample in an object of tye USData if it is closer to one of two groups
#' 
#' @param us an object of type USData
#' @param group a vector of characters. this vector needs to have an entry for each sample within the USdata and exactly two groups
#' @param lab character. the label group for which the group likeness ratio will be calculated
#' @return an object of type USData with added metadata (distance to group 1, distance to group 2 and ratio of the distances)
#' @examples
#' US <- StratifyGroups(US, group = c("control","control","control","test","test","test"), lab = "kmeans.50")
#' US <- StratifyGroups(US, group = US$meta$Treatment, lab = "kmeans.50)

StratifyGroups <- function(us, group, lab){
  require(imputeTS)
  require(pracma)
  if(length(us$files) != length(group)){
    warning("grouping vectorn needs to have same length as number of files. can not perform analysis\n")
  ...
  group <- sapply(group, FUN = function(x){ gsub("[[:space:]]", ".",x)}) #this fixes an issue with colnames because of automatic replacement of spaces with "." by R that then do not correlate to the "true" names anymore. we just force the replacement before any operations
  results <- data.frame()
  
  #this loop iterates through files, sets aside the file to be assessed and then compares its distance to the median transition matrix of both groups
  for(i in 1:length(us$file_names)){
    mat <- us$files[[i]]$transmatrix[[lab]]
    fs <- us$file_names[-i]
    gs <- group[-i]
    transmat <- list()
    for(j in fs){
      transmat[[j]] <- us$files[[j]]$transmatrix[[lab]]
    }
    
    A <- transmat[gs == unique(group)[1]]
    B <- transmat[gs == unique(group)[2]]
    
    a <- do.call(cbind, A)
    a <- array(a, dim=c(dim(A[[1]]), length(A)))
    a <- apply(a, c(1, 2), median, na.rm = TRUE)
    
    b <- do.call(cbind, B)
    b <- array(b, dim=c(dim(B[[1]]), length(B)))
    b <- apply(b, c(1, 2), median, na.rm = TRUE)
    
    dist_a <- sum(abs(a - mat))
    dist_b <- sum(abs(b - mat))
    
    #add output to results data.frame
    newrow <- list()
    newrow[paste("distance",unique(group)[1],sep = "_")] <- dist_a
    newrow[paste("distance",unique(group)[2],sep = "_")] <- dist_b
    newrow$ratio <- log(dist_a / dist_b)
    results <- rbind(results,newrow)
  }
  
  #check if any samples are "missclassified"
  results$missclassified <- FALSE
  results$missclassified[(us$meta$group == unique(group)[1] & results$ratio > 1)] <- TRUE
  results$missclassified[(us$meta$group == unique(group)[2] & results$ratio < 1)] <- TRUE
  results$grouping_missclassified <- ifelse(results$missclassified, paste(group,"_missclassified", sep = ""),group)
  
  #add results to metadata
  us$meta <- cbind(us$meta,results)
  return(us)
}
...
# DistinctionThresFor2DEmbedding <- function(){
#   # Get X1 and X2 from 2D embedding
#   # Choose coordinate (either X1 or X2) which separates two groups best based on AUC of PR curve
#   # Get best threshold (= highest F1 score) for first coordinate
#   # Get 2D embedding coordinates for area where defined group is over represented (either higher or lower than threshold)
#   # Get best threshold for second coordinate
#   # Return thresholds
# }
...
PlotTransitionsStats <- function(us, analysis = NULL, labels = NULL){
  ...
  for(i in analysis){
    for(j in labels[labels %in% names(us$Results[[i]]$TransitionStats)]){
      ...
      cohensD <- us$Results[[i]]$TransitionStats[[j]]$CohensD
      

      tryCatch({
        effect_size <- us$Results[[i]]$Statistics[[j]]
        effect_size <- na.omit(effect_size[effect_size$p < 0.05,"FC"])
        effect_size <- ifelse(effect_size < 1, 1/effect_size, effect_size)
        effect_size <- median(effect_size)
      }, error = function(e){
        effect_size <- NA
      })
      if(((1 - pracma::erf(sigma / sqrt(2))) / 2) > 0.05){
        effect_size <- NA
      }
      ...
      p2 <- ggplot() + 
        ggtitle(paste("percentile:", signif(percentile,digits = 4), "\n", "sigma:", round(sigma,digits = 2), "\n", "p.value:", signif((1 - pracma::erf(sigma / sqrt(2))) / 2,digits = 3) , "\n", "median absolute FC:", round(effect_size, digits = 2) , "\n", "Cohen's d:", round(cohensD, digits = 2) , sep = " ")) + 
        theme_minimal() +
...
PlotClusterStats <- function(us, analysis = NULL, labels = NULL){
  require(ggplot2)
  require(cowplot)
  if(is.null(analysis)){
    analysis <- names(us$Results)
  ...
  if(is.null(labels)){
    for(i in analysis){
      labels <- unique(c(labels, names(us$Results[[i]]$ClusterStats)))
    }
  }
  
  ps <- list()
  for(i in analysis){
    for(j in labels[labels %in% names(us$Results[[i]]$ClusterStats)]){
      bdat <- data.frame(distance = us$Results[[i]]$ClusterStats[[j]]$bootstraps)
      dis <- us$Results[[i]]$ClusterStats[[j]]$distance
      sigma <- us$Results[[i]]$ClusterStats[[j]]$sigma
      percentile <- us$Results[[i]]$ClusterStats[[j]]$percentile
      p1 <- ggplot(bdat,aes(distance)) + 
        geom_histogram(color = "black", fill = "gray70", bins = 30) + 
        geom_vline(xintercept = dis, color = "red") +
        theme_bw() + 
...
PlotTransitions <- function(us, from, to, lab, groupby = NULL, cols = NULL){
  pdat <- NULL
  for(i in names(us$files)){
    dat <- us$files[[i]]$transmatrix[[lab]]
    grp <- NA
    if(!is.null(groupby)){
      grp <- us$meta[i,groupby]
    }
    for(idx in 1:length(from)){
      j = from[idx]
      k = to[idx]
      pdat <- rbind(pdat, data.frame(name = i, from = j, to = k, transition = paste(j,k,sep = " -> "), value = dat[j,k], group = grp))
    }
  }
...
#' @param random if TRUE, examples are picked randomly from each video
#' @param output.folder name of output folder, will be created in working directory
#' @param exp.name name of experiment that is used at the beginning of each output video
  ...
  for (l in labels) {
    ...
    for (c in clusters) {
      ...
      for (v in list_videos) {
        # Check if video is contained in the US object
        if(!is.element(TRUE, grepl(v, us$meta$VideoFile, fixed = TRUE)))  {
          print(paste("Video", v, "is not contained in the US object."))
          next
        }
        # Check if name of video appears several times in US object
        if(sum(grepl(v, us$meta$VideoFile, fixed = TRUE)) > 1)  {
          print(paste("Video", v, "matches with several file names in the US object."))
          next
        }
        
        name_v <- v
        v <- us$file_names[grepl(v, us$meta$VideoFile, fixed = TRUE)]
        
        # Check if current cluster appears in current video
        if(!(c %in% names(us$files[[v]]$OnSetData[[l]]))) {
          print(paste("Cluster", c, "does not appear in video", v, "."))
          next
        }
        ...
        # Remove examples if they are shorter than min.length (in seconds!)
        if (min.length.s > 0) {
          tooShort <- (examples[, 2] - examples[, 1]) < min.length.s
          examples <- examples[!tooShort, ]
        }
        ...
        input <- paste(path.videos, name_v, video.format, sep = "")
        for(e in 1:nrow(examples)){
          output <- paste("tmp\\",v,"_",e,".mp4", sep = "")
          command <- paste("ffmpeg -ss", examples[e,1], 
                           "-to", examples[e,2],
                           "-i", input, 
                           output,
                           sep = " ")
      ...
      # Stitch example videos together
      ex_videos <- list.files("tmp/")
      readr::write_delim(data.frame("file",paste("'tmp\\",ex_videos,"'",sep = "")), "inputs.txt", col_names = F)
      name_output_video <- paste(exp.name, "_", l, "_C", c, "_n", n, ".mp4", sep = "")
      system(paste("ffmpeg -f concat -safe 0 -i inputs.txt -vcodec copy -acodec copy ", output.folder, "\\", name_output_video, sep = ""))
...
cohensD <- function(x, y) {
  ...
  sd_pooled <- sqrt(((nx - 1) * sd(x)^2 + (ny - 1) * sd(y)^2) / (nx + ny - 2))
...
#' Calculates the weighted Cohen's D for all results in an object of type USData
#' 
#' @param us an object of type USdata
#' @return data frame of weighted cohen's D values
#' 
CohensD_weighted <- function(us){
  Res <- data.frame()
  for(j in names(us$Results)){
    for(i in names(us$Results[[j]]$Statistics)){
      cohend_weight <- us$Results[[j]]$Statistics[[i]]
    tryCatch({
      cohend_weight <- na.omit(cohend_weight)
      es <- cohend_weight$CohensD
      es <- ifelse(es < 0, -es, es)
      weights <- 1-sigmoid(cohend_weight$p, a = 50, b = 0.05)
      cohend_weight <- sum(es * weights) / sum(weights)
      #cohend_weight = max(es)
    }, error = function(e){
      cohend_weight <- NA
    })
      Res <- rbind(Res, data.frame(test = j, label = i, category = "Usage", weighted_cohenD = cohend_weight))
    }
  }
  
  for(j in names(us$Results)){
    for(i in names(us$Results[[j]]$TransitionStats)){
      cohend_weight <- us$Results[[j]]$TransitionStats[[i]]$transitions
      tryCatch({
        cohend_weight <- na.omit(cohend_weight)
        es <- cohend_weight$CohensD
        es <- ifelse(es < 0, -es, es)
        weights <- 1-sigmoid(cohend_weight$p.value, a = 50, b = 0.05)
        cohend_weight <- sum(es * weights) / sum(weights)
        #cohend_weight = max(es)
      }, error = function(e){
        cohend_weight <- NA
      })
      Res <- rbind(Res, data.frame(test = j, label = i, category = "Transitions", weighted_cohenD = cohend_weight))
    }
  }
  return(Res)
}

#' Performs a power analysis for an object of type USData. Exactly one of the parameters,'n','power' and 'sig.level' must be passed as NULL (by default n), and that parameter is determined from the others
#' 
#' @param us an object of type USdata
#' @param n the number of samples per group (defaults to NULL = what we want to calculate)
#' @param sig.level the significance level that has to be achieved by the experiment (defaults to 0.05)
#' @param power the power that has to be ensured by the experiment (defaults to 0.8)
#' @param condensed if FALSE plots different sig levels in a plot for each test, if TRUE plots different tests for each sig level in a plot
#' @return a list of power calculation results for each test and each label
#' @examples
#' what group size do we need to have a 50% chance of finding a significant effect with a p < 0.05?
#' PowerAnalysis(us = US, sig.level = 0.05, power = 0.5)
#' 
#' if we perform an new experiment with a group size of 16 and want to find a p < 0.05, what is the power of the analysis?
#' PowerAnalysis(us = US, n = 16, sig.level = 0.05, power = NULL)
#' 
#' if we perform an new experiment with a group size of 16, what significance can we expect with a power of 0.9?
#' PowerAnalysis(us = US, n = 16, sig.level = NULL, power = 0.9)
#' 
PowerAnalysis <- function(us, n = NULL ,sig.level = 0.05, power = 0.9){
  res <- list()
  for(j in names(us$Results)){
    for(i in names(us$Results[[j]]$TransitionStats)){
      d = us$Results[[j]]$TransitionStats[[i]]$CohensD
      res[[paste(i,j, sep = "_")]] <- pwr.t.test(n = n, sig.level = sig.level, d = d, power = power, type="two.sample", alternative="two.sided")
    }
  }
  return(res)
}

PlotPowerAnalysisCurves <- function(us, n_range = 3:100, sig.levels = c(0.05, 0.01, 0.001, 0.0001), condensed = FALSE){
  results <- data.frame()
  for(j in names(us$Results)){
    for(i in names(us$Results[[j]]$TransitionStats)){
      for(n in n_range){
        for(s in sig.levels){
          p = pwr.t.test(n = n, sig.level = s, d = us$Results[[j]]$TransitionStats[[i]]$CohensD, power = NULL, type="two.sample", alternative="two.sided")$power
          results <- rbind(results, data.frame(n = n, sig = s, test = j, label = i, power = p, analysis = paste(j, i, sep = "_")))
        }
      }
    }
  }
  if(condensed){
    plot <- ggplot(results, aes(n, power, color = analysis)) + geom_line() + facet_wrap(~as.factor(sig)) + theme_bw()
  }
  else{
    plot <- ggplot(results, aes(n, power, color = as.factor(sig))) + geom_line() + facet_wrap(~analysis) + theme_bw()
  }
  return(plot)
}
