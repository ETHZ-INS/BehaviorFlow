...
LDBAnalysis <- function(t, movement_cutoff,integration_period, points){
  ...
  for(k in points){
    ...
    #transitions from light to dark only
    light_dark <- ifelse(IsInZone(t,k,"light"), "light","background")
    light_dark <- ifelse(IsInZone(t,k,"dark"), "dark", light_dark)
    light_dark <- SmoothLabel(light_dark, integration_period)
    
    light_dark_transitions <- 0
    for(i in 1:(length(light_dark) - 1)){
      if(light_dark[i] == "light" & light_dark[i+1] == "dark"){
        light_dark_transitions <- light_dark_transitions + 1
      }
    }
    
    #transitions from dark to light
    dark_light_transitions <- 0
    for(i in 1:(length(light_dark) - 1)){
      if(light_dark[i] == "dark" & light_dark[i+1] == "light"){
        dark_light_transitions <- dark_light_transitions + 1
      }
    }
    
    t$Report[[paste(k, ".transitions.light.to.dark")]]  <- light_dark_transitions
    t$Report[[paste(k, ".transitions.dark.to.light")]]  <- dark_light_transitions
    t$Report[[paste(k, ".transitions.total")]]  <- light_dark_transitions + dark_light_transitions
    t$Report[[paste(k, "latency.to.entry.dark")]] <- LatencyToEntry(t,k,"dark")
  }
  return(t)
}
...
CreateSkeletonData_LDB_v2 <- function(t){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  dat <- data.frame(Ac1 = t$data[["nose"]]$acceleration)
