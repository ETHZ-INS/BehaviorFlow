...
RenderClusterVideos <- function(us, labels, cluster.values, path.videos, video.format = ".mp4", n = NULL, random = TRUE, output.folder = "Examples", exp.name = "Exp", fps, min.length.s = 0, lag.s = 0){
  ...
  for (l in labels) {
    ...
    for (c in clusters) {
      ...
      write_delim(data.frame("file",paste("'tmp\\",ex_videos,"'",sep = "")), "inputs.txt", col_names = F)
      name_output_video <- paste(exp.name, l, c, n, ".mp4", sep = "_")
      system(paste("ffmpeg -f concat -safe 0 -i inputs.txt -vcodec copy -acodec copy ", output.folder, "\\", name_output_video, ".mp4", sep = ""))
