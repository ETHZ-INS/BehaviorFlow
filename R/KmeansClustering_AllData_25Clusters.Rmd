---
title: "KmeansClustering_AllData_25Clusters"
...
Due to memory limits we do not want to run the unsupervised clustering on all files. Instead, we select 20 files randomly from each experiment used for the "original" clustering.
...
We are now ready to run the unsupervised clustering. We run k-means on the selection of data for 4 different numbers of clusters (10, 25, 50, 100).
...
We repeat the same clustering, this time after dimensionality reduction using SVD.
...
Next, we create an unsupervised analysis dataobject for the datasets used for the original clustering. We add the metadata that contains the experimental details and save it as one object.

```{r, eval = FALSE}
US <- LoadFromTrackingObject(ts)
meta <- read.table("../metadata/CSI_AS_Yoh_25Clusters.csv", sep = ";", header = T)
rownames(meta) <- meta$DLCFile
meta$DLCFile <- NULL

US <- AddMetaData(US, meta)

saveRDS(US,"../data/US_CSI_AS_Yoh.rds")
saveRDS(ts,"../data/TS_CSI_AS_Yoh.rds")

ts <- readRDS("../data/TS_CSI_AS_Yoh.rds")
```

We additionally save a data object that contains both original results and clustering transfer results from new experiments (DREADD, CRS).
...
saveRDS(US_transfer,"../data/US_AllData.rds")
```